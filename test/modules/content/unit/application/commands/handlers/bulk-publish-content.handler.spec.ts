import { BulkPublishContentHandler } from '../../../../../../../src/modules/content/application/commands/handlers';
import { BulkPublishContentCommand } from '../../../../../../../src/modules/content/application/commands';
...
describe('BulkPublishContentHandler', () => {
  let handler: BulkPublishContentHandler;
  ...
  beforeEach(() => {
    ...
    handler = new BulkPublishContentHandler(mockRepository, mockCacheService);
  });

  describe('execute', () => {
    it('should publish multiple approved contents', async () => {
      // Arrange
    ...
    it('should handle domain exceptions during publish', async () => {
      // Arrange
      const content = Content.reconstitute({
        id: 'content-1',
        tenantId: 'tenant-1',
        ...
        excerpt: null,
        type: ContentType.article(),
        status: ContentStatus.draft(), // Not approved - will throw exception
        priority: ContentPriority.medium(),
        categoryId: null,
        tags: [],
        featuredImage: null,
      ...
      expect(result.results[0].error).toContain('Cannot publish content');
      expect(mockRepository.save).not.toHaveBeenCalled();
    });
    ...
    it('should invalidate cache for each content before publishing', async () => {
      // Arrange
      const content = Content.reconstitute({
        id: 'content-1',
        tenantId: 'tenant-1',
