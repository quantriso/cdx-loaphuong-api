...
import { BulkPublishContentCommand } from '../bulk-publish-content.command';
...
import type { BulkOperationResult } from '../bulk-publish-content.command';
import { DomainException } from '@core/common';
import { Content } from '../../../domain/entities';
import {
  BulkContentPublishedEvent,
  type BulkContentPublishItem,
} from '../../../domain/events/bulk-content-published.event';

/**
 * Bulk Publish Content Handler
 *
 ...
 */
@CommandHandler(BulkPublishContentCommand)
export class BulkPublishContentHandler implements ICommandHandler<
  BulkPublishContentCommand,
  BulkOperationResult
> {
  private readonly logger = new Logger(BulkPublishContentHandler.name);
  ...
  async execute(
    command: BulkPublishContentCommand,
  ): Promise<BulkOperationResult> {
    const { items, adminId, tenantId, options } = command;

    this.logger.log(
      `Executing bulk publish for ${items.length} contents in tenant ${tenantId}`,
    );
    ...
    const { results, warnings, successfulOperations, shouldRollback } =
      this.bulkOperationsService.processBulkPublish(
        items,
        contentMap,
        serviceOptions,
        adminId,
    ...
    if (shouldRollback) {
      ...
      throw new DomainException(
        `Bulk publish failed: ${failedCount} operations failed. All changes rolled back.`,
      );
    }
    ...
    try {
      ...
      if (savedContents.length > 0) {
        const bulkEventData: BulkContentPublishItem[] = savedContents.map(
        ...
        const bulkEvent = new BulkContentPublishedEvent(
        ...
        this.logger.log(
          `Bulk publish completed: ${savedContents.length}/${items.length} successful`,
        );
      }
